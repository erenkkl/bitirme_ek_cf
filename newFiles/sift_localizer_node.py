...
class SiftLocalizerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('reference_csv_path', '/home/fidan/bitirme_dataset/references.csv')
        ...
        self.ref_csv_path_history = os.path.join(os.path.expanduser("~"), "flight_dataset/references.csv")
        ...
        # ORB uses binary descriptors, so we use BFMatcher with Hamming distance
                        ...
                        if img is not None:
                            ...
                            if des is not None and len(kp) > 5:
                                ...
                                new_db.append({
                                    'seq': int(row['seq']),
                                    'kp': kp,
                                    'des': des,  # ORB descriptors are already uint8
                                    'rec_conf': rec_conf,
                                    'map_seq': map_seq
                                })
    ...
    def image_callback(self, msg):
        ...
        try:
            ...
            if des_live is None:
                return
            # ORB descriptors are already in the correct format (uint8)
        except:
            return
